...
function UpdateBeritaAdmin() {
  const location = useLocation();
  const { id, judul, konten, image } = location.state || {};
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/admin/updateberita/${id}`, beritaData, {
        headers: {
          ...
          <div className="bg-gray-200 p-10 rounded-lg shadow-md">
            <form onSubmit={handleSubmitClick} id="form-tambah-berita" className="grid grid-cols-1 gap-6">
              ...
              <div className="flex space-x-4 mt-6">
                ...
                >
                  {isSubmitting ? 'Submitting...' : 'Update Berita'}
                </button>
              </div>
            </form>
          </div>
  ...
  <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/3 max-w-full">
      <h3 className="text-lg font-semibold">Konfirmasi Update</h3>
...
export default UpdateBeritaAdmin;
